...
make_dist(){
    ...
    tar_file=${target_folder}.tar
    tar_timestamp_file=${target_folder}-${TIMESTAMP}.tar
    ...
    execute pushd ${BUILD_FOLDER}
        echo "Creating $target_tar from $target_folder."
        execute tar -cf $target_tar $target_folder
    execute popd

    # Create symlink.
    execute pushd ${DIST_FOLDER}/${kind}
        execute ln -sf ${OS}/${ARCH}/${tar_timestamp_file} ${tar_file}
    execute popd
}
